...
var logger = log4js.getLogger("WebApp_CC");
...
var upgrade = require("./app/upgrade-chaincode");
var invoke = require("./app/invoke-chaincode");
...
const totalTransaction = new prometheus.Counter({ name: 'total_transaction', help: 'Counter for total transaction' })
const failedTransaction = new prometheus.Counter({ name: 'failed_transaction', help: 'Counter for failed transaction' })
const successfulTransaction = new prometheus.Counter({ name: 'successful_transaction', help: 'counter for successful transaction' })
...
app.post("/users", async function (req, res) {
  ...
  if (response && typeof response !== "string") {
    ...
    logger.debug("Generated JSON Web Token", response.token);
    // res.json(response.token);
    //res.json({ success: true, token: response })
    res.json({ result: response, error: null, errorData: null });
  } else {
    logger.debug(
    ...
    res.json({
      result: null,
      error: "Error",
      errorData: "Failed to register the user"
    });
  }
});
...
app.post("/channels/:channelName/chaincodes/:chaincodeName", async function (
  ...
  try {
    ...
    successfulTransaction.inc(1)
    res.send(response_payload);
  } catch (error) {
    failedTransaction.inc(1)
    const response_payload = {
      result: null,
      error: error.name,
