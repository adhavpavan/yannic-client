...
var invokeChaincode = async function (
  peerNames,
  channelName,
  chaincodeName,
  fcn,
  ...
  try {
    ...
    // console.time("AAAAA Get client")
    var client = await helper.getClientForOrg(org_name, username);
    // console.timeEnd("AAAAA Get client")
    logger.debug(
      'Successfully got the fabric client for the organization "%s"',
      org_name
    );
    console.time("AAAAA Get Channel")
    var channel = client.getChannel(channelName);
    console.timeEnd("AAAAA Get Channel")
    if (!channel) {
    ...
    // console.time("AAAAA Create Transaction id")
    var tx_id = client.newTransactionID();
    // will need the transaction ID string for the event registration later
    tx_id_string = tx_id.getTransactionID();
    // console.timeEnd("AAAAA Create Transaction id")
    ...
    console.log("============TESTING INVOKE-CHAINCODE.JS=================");
        ...
        let invokeEventPromise = new Promise((resolve, reject) => {
          ...
          eh.registerTxEvent(
            ...
            (tx, code, block_num) => {
              ...
              logger.info("Event Hub connection status %s", eh.isconnected());
              logger.info(
                "Transaction %s has status of %s in block %s",
                tx,
                code,
                block_num
              );
